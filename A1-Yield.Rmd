...
On the Y axes is the values of the coefficients and on the X axes the L1 Norm is the sum of the 
...
The coefficient plot outcome is hard to interpret because of the axes scale. Depending on the 
choice of tuning parameter, some of the coefficients are exactly equal to zero
```{r}
...
```
This value is correspondent to the right dashed line in the plot. It is the suggested trade 
...
to the optimization criteria (RSS) and uses its absolute values. 
